...
m=1         ##number of exposures
strength_IV=1 ##1  ##a way of regulating strength of IV (this is the most powerful way of generating weak IV)
...
for (j in 1:k)
{
  ...
  aa<- runif(1, min = -1, max = 1)  ##sample(matrix(cbind(-1,1)),1)*
  
  for (i in 1:(m+1))
  {
    lmk[i,j]<-sample(cbind(-1,1),1)*aa+rnorm(1, mean = 0, sd = 0.1)
  }
}
...
u_mean=c(1,1) ##scenario 1

##u_mean=c(-1,1,1,1) ##scenario 2
##u_mean=c(1,-1,1,1) ##scenario 3
##u_mean=c(1,1,-1,1) ##scenario 4
...
for (i in 1:m)
{
  lp[1]=1 ##change risk factor here
...
data$y<-data$y_sub_a1+rowSums(data[,c(paste0("y_sub_snp", 1:k))])
+ u[,m]  
+ rnorm(n, mean = 0, sd = 1)
