...
colnames(IV_strength)<-rbind("IV_x1","IV_x2","IV_x3")
...
source('/Users/simulation2.data.r')
  ...
  IV_strength[simu_loop,1]<-table(new_data_f_di[,1])[2]
  IV_strength[simu_loop,2]<-table(new_data_f_di[,2])[2]
  IV_strength[simu_loop,3]<-table(new_data_f_di[,3])[2]
...
write.csv(IV_strength,"./table1_IV.csv")
