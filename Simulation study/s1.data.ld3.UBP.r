...
##estimate correlation of exposures using LD-score regression, which will be used for MVMR
...
##check whether direct and effect mediated through a are independent
cor(rowSums(data[,c(paste0("y_sub_a", 1:exp_num))]),rowSums(data[,c(paste0("y_sub_snp", 1:snp_num))]))

##check INSIDE assumption

cor(lmk_x[1,], lmk_y_unbal[1,])
cor(lmk_x[2,], lmk_y_unbal[1,])
cor(lmk_x[3,], lmk_y_unbal[1,])
